...
Write a clear stock-only research report from the supplied company evidence packet.

Reader-output rule:
- The final report is for an investor who wants to understand only the stock. Do not mention data providers, connector names, backend systems, raw sync failures, app errors, MCP, API, payloads, table headers, JSON, OCR fragments or data-pipeline issues.
- If evidence is thin, contradictory or unreconciled, translate that into investment language: "valuation confidence is low", "ownership trend requires monitoring", "margin evidence needs confirmation", or "verdict withheld until share-basis math is reconciled." Do not explain which provider/source failed.
- You may refer generically to "company disclosures", "public filings", "market data", "ownership filings", "recent announcements" or "available evidence" when needed.
...
- Use this evidence hierarchy internally: audited annual filings and exchange filings first, structured fundamentals and ownership data second, market prices third, derived calculations fourth. Do not expose that hierarchy as provider names in the final report.
...
- Include these sections: Sanity check before verdict, Executive verdict, Business quality, Financial quality, Valuation, Growth runway, Ownership, Risks, Catalysts, Multibagger probability, Trap probability, Evidence confidence, Needs verification, Final recommendation, What must happen for 5x/10x, What would make this fail.
...
- Always provide a provisional investment view, confidence score and next evidence requests. A data gap lowers confidence; it should not turn the report into an empty refusal unless core share-basis math cannot be reconciled.
- Do not output Trap probability 100/100 or Multibagger probability 100/100. Use calibrated 5-95 ranges and explain the drivers. If two or more critical evidence checks fail, write "Verdict withheld - evidence confidence is too low."
- Never repeat raw payloads, table headers, OCR fragments, JSON-like dumps or truncated evidence blocks in the final report. Summarize clean evidence only and move messy source output to Needs verification.
